...
function solution(n, m, section) {
  let answer = 0; // 페인트칠해야 하는 최소 횟수
  let currentIndex = 0; // 현재 인덱스

  for (const sect of section) {
    // 다시 칠해야 하는 구역
    if (currentIndex < sect) {
      // 현재 인덱스가 다시 칠해야 하는 구역보다 작으면
      answer++; // 페인트칠해야 하는 최소 횟수 증가
      currentIndex = sect + m - 1; // 현재 인덱스를 다시 칠해야 하는 구역 + 롤러 길이 - 1로 설정
    }
  }
  return answer;
}
